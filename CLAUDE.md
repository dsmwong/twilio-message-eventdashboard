...
- `_shared/conversations-api.js` — raw HTTPS client for the Twilio Communications/Conversations v2 API at `https://conversations.twilio.com/v2/...` (HTTP Basic with ACCOUNT_SID/AUTH_TOKEN, JSON-only). Exports `getConversation(context, id)` and `listCommunications(context, id)` (paginated). Used by `orchestrator-callback.js`.
